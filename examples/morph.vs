...
{
  if (a.distance > -b.distance) return a;
  return Obj(-b.distance, b.material);
}
...
float sdf_box(vec3 pos, vec3 size, vec3 ray_tip) {
    vec3 p = abs(ray_tip + pos)-size;
...
void map_obj(out Obj obj, vec3 ray_tip)
{
  ...
  Obj plane		   = Obj(plane_dist, 1);

  float cube_dist = sdf_box(vec3(0.0), vec3(1.0), ray_tip);
  Obj cube = Obj(cube_dist, 2);
  
  Obj sphere = Obj(length(ray_tip) - 2.0 * abs(sin(iTime*0.4)), 2);

  cube = mask_obj(cube, sphere);
  obj = merge_obj(plane, cube);
}


vec3 get_normal(vec3 p)
{
  /* Feel free to laught at me and this normal function... */
...
vec3 calc_light(Obj obj, Ray cam, vec3 pos)
{
  ...
  return (dist < length(pos-ray_tip)) ? ambient: light+highlight*0.7+ambient;
}
...
void main()
{
	...
	Ray cam = get_cam(vec3(3.0, 3.0, -3.0), vec3(0.0), uv, FOV);
